...
//		private var prevElementX:Number = 0;
		...
		private var prevBlockNumber:uint; // prevent generating the same block again
		private var coinGenerator:CoinPatternGenerator;
		private var stageWidth:int;
		private var stageHeight:int;
		
		public function LevelGenerator(builder:LevelParser) {
			this.builder = builder;
			this.coinGenerator = new CoinPatternGenerator(this.builder);
		}
		
		public function initialize():void {
			this.stageWidth = Statics.stageWidth;
			this.stageHeight = Statics.stageHeight;
			this.prevBlockNumber = 0;
			this.coinGenerator.initailize();
			this.rowsToSkip = 0;
		}
		
		public function generate(type:uint):void {
			if (type == this.prevBlockNumber) return;
			this.prevBlockNumber = type;
			
//			trace("generating block number: " + type);
			Statics.levelNumber = type; // mixpanel game over data
			...
			var height:int = 50; // height of block to generate
...
//					break;
				...
				case 800:
					...
					// turn off power duplication
//					this.builder.levelElementsArray.push([this.builder.currentY * Constants.UnitHeight, Constants.PowerSettingDuplication, false]);
...
//					break;
				...
				case 4001: // designed pattern
					...
					break;
				case 4002:
					this.generateStairWithName("PlatformNormalBoost", 4, 2);
					break;
				case 4003:
...
//					break;
				...
				case 5000: // [2] size [3-4] [normal, drop, mobile, normalboost, dropboost, mobileboost, bouncer, super]
					this.prevBlockNumber = 0; // only one block type for this difficulty, therefore allow immediate repeat
				...
				case 7000:
					this.generateDesignedPlatformAndReuplsor(1);
					break;
				case 7001:
				...
				case 8004:
					...
					break;
				case 8005:
					this.generateStairWithName("PlatformMobile", 4);
					break;
				...
				case 12004:
					...
					break;
				case 12005:
					this.generateStairWithName("PlatformMobileBoost", 4, 2);
					break;
				...
				case 14003:
					...
					break;
				case 14004:
					this.generateStairWithName("Bouncer", 4, 2);
					break;
...
//					break;
		...
		private function generateRandom(height:int, repulsorWalls:Boolean, elementDistribution:Array, 
			...
			while (this.builder.currentY < roof) { // one row of platforms
				if (this.rowsToSkip > 0) { // skip row
					this.rowsToSkip--;
				} else { // do not skp row; add platform(s)
					for (var i:uint = 0; i < elementsPerRow; i++) { // loop through elements on the same row
						var border:Number = this.stageWidth / 15;
						if (elementsPerRow == 2 && i == 0) { // left element
							gx = Math.random() * (this.stageWidth / 2 - border * 2) - (this.stageWidth / 2 - border);
						}
						...
						//						this.prevElementX = -this.stageWidth / 2 + 300;
						//					}
						//					else if (gx > this.stageWidth / 2 - 300) {
						//						this.prevElementX = this.stageWidth / 2 - 300;
						//					}
						...
						if ([Constants.PlatformNormal, Constants.PlatformDrop, Constants.PlatformMobile].indexOf(elementClass) != -1) {
...
//							if (Math.random() < 0.5) this.rowsToSkip = 6;
//						}
						...
						//						elementSize = this.getElementSizeByDistribution(sizeDistribution);
						//					}
						// make platforms smaller on repetitions
						if (this.builder.repetitions > 0) {
							if (this.builder.repetitions > 1) elementSize -= 2;
							else elementSize--;
							if (elementSize < 2) elementSize = 2;
						}
...
//				this.coinGenerator.addRow();
//				this.builder.currentY++;
//				this.coinGenerator.addRow();
//				this.builder.currentY++;
//				this.coinGenerator.addRow();
				this.builder.currentY++;
				this.coinGenerator.addSimplePattern();
				this.builder.currentY++;
				this.addRedStarsAtRandom(0.03);
				this.builder.currentY++;
			} // eof one row
		} // eof generateRandom()
...
//		}
		...
		private function generateSingleRisingMobile(difficulty:uint = 1):void {
			...
			if (difficulty < 3) {
				...
				var separationX:Number = (this.stageWidth) / 12;
				for (i = 0; i < 12; i++) {
					gx = 35 + i * separationX - (this.stageWidth) / 2;
		...
		private function generateDesigned2000(difficulty:uint = 1):void {
			...
			if (difficulty == 9) { // add a row of red stars to make things easier
				...
				for (i = 0;  i < numElementsPerRow; i++) { // loop through elements on the same row
					...
					gx = (this.stageWidth / (numElementsPerRow + 1)) * (i + 1) - this.stageWidth / 2;
					gy = this.builder.currentY * Constants.UnitHeight;
			...
			for (var ri:uint = 0; ri < 9; ri++) {
				...
				for (var i:uint = 0;  i < numElementsPerRow; i++) { // loop through elements on the same row
					...
					if (prevType == 1)
						...
						gx = (this.stageWidth / (numElementsPerRow + 1)) * (i + 1.5) - this.stageWidth / 2;
					}
					
					if (difficulty == 1) {
						if (Math.random() < 0.2) this.builder.levelElementsArray.push([gy, gx, "StarBlue"]);
						else this.builder.levelElementsArray.push([gy, gx, "Star"]);
					}
					else if (difficulty == 2) {
						if (Math.random() < 0.2) this.builder.levelElementsArray.push([gy, gx, "Repulsor"]);
						else this.builder.levelElementsArray.push([gy, gx, "Star", 0]);
					}
					else if (difficulty == 3) {
						if (Math.random() < 0.2) this.builder.levelElementsArray.push([gy, gx, "StarDark"]);
						else this.builder.levelElementsArray.push([gy, gx, "Star", 0]);
					}
				} // eof loop through elements on the same row
...
//				}
		...
		private function generateSineWave(difficulty:uint = 1):void {
			var gx:Number = 0;
			var gy:Number = 0;
			for (var ri:uint = 0; ri < 40; ri++) { // loop through rows
				var numElementsPerRow:int = 2;
				gy = this.builder.currentY * Constants.UnitHeight;
				for (var i:uint = 0;  i < numElementsPerRow; i++) { // loop through elements on the same row
					// get new gx and gy values for new element
					gx = ((this.stageWidth - 100) / 2) * Math.sin(this.builder.currentY / 8) + (i - 0.5) * 100;
				...
				// add coin
				this.builder.currentY ++;
				this.builder.levelElementsArray.push([this.builder.currentY * Constants.UnitHeight, ((this.stageWidth - 100) / 2) * Math.sin(this.builder.currentY / 8), Constants.Coin]); // coin
				this.builder.currentY ++;
			}
			// two blue stars at the end
			gy = this.builder.currentY * Constants.UnitHeight;
			gx = ((this.stageWidth - 100) / 2) * Math.sin(this.builder.currentY / 8) + (-0.5) * 100;
			this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0]);
			gx = ((this.stageWidth - 100) / 2) * Math.sin(this.builder.currentY / 8) + (0.5) * 100;
			this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0]);
			
			this.builder.currentY += 4;
			this.addRedStarsAtRandom(0.1);
		}
		...
		private function generateDesigned2003(difficulty:uint = 1):void {
			...
			for (var ri:uint = 0; ri < 80; ri++) {
				...
				if (gx > this.stageWidth / 2 - 50) gx = this.stageWidth / 2 - 50;
				else if (gx < -this.stageWidth / 2 + 50) gx = -this.stageWidth / 2 + 50;
				prevGx1 = gx;
				gy = this.builder.currentY * Constants.UnitHeight;
				this.builder.levelElementsArray.push([gy, gx, "Star"]);
				// line 2
				...
				if (randomNum < 0.2) {
					gx = Math.random() * 600 - 300;
					if (Math.abs(gx - prevGx1) > 70 && Math.abs(gx - prevGx2) > 70) this.builder.levelElementsArray.push([gy, gx, "PlatformDrop", 4]);
				}
		...
		public function addRowRedStars():void {
			// disable power duplication for this
			this.builder.levelElementsArray.push([this.builder.currentY * Constants.UnitHeight, Constants.PowerSettingDuplication, false]);
...
//			if (Math.random() > 0) { // add two rows of coins
				for (var ri:uint = 0; ri < 2; ri++) {
					...
					for (var i:uint = 0;  i < numElementsPerRow; i++) { // loop through elements on the same row
						// get new gx and gy values for new element
						var gx:Number = (this.stageWidth / (numElementsPerRow + 1)) * (i + 1) - this.stageWidth / 2;
...
//		}
		...
		private function generateDesignedPlatformAndReuplsor(difficulty:uint = 1):void {
			var gx:Number = 0;
			var gy:Number = 0;
			for (var ri:uint = 0; ri < 12; ri++) { // loop through rows
				gy = this.builder.currentY * Constants.UnitHeight;
				// left platform
				if (difficulty == 1) {
					gx = -250;
					if (this.builder.repetitions > 1) this.builder.levelElementsArray.push([gy, gx, "PlatformMobileBoost", 3]);
					else this.builder.levelElementsArray.push([gy, gx, "PlatformMobileBoost", 4]);
				}
				else if (difficulty == 2) {
					gx = Math.random() * 200 - 350;
					if (this.builder.repetitions > 1) this.builder.levelElementsArray.push([gy, gx, "PlatformMobileBoost", 2]);
					else this.builder.levelElementsArray.push([gy, gx, "PlatformMobileBoost", 3]);
				}
...
//				this.builder.currentY++;
		...
		private function generateBeginnerSpinningCircles(difficulty:uint = 1):void { //
			...
			var gy:Number;
			var j:uint;
			for (var i:uint = 0; i < 6; i++) { // loop through rows of spinners
				gx = Math.random() * 400 - 200;
				gy = this.builder.currentY * Constants.UnitHeight;
		...
		private function generateMickeyMouseLarge():void {
			...
			for (var i:uint = 0; i < 10; i++) { // loop through rows
				...
				gy = (this.builder.currentY - 0.5) * Constants.UnitHeight;
				this.builder.levelElementsArray.push([gy, gx, "PlatformDrop", 3]);
		...
		private function generateEasySpinningCircles():void { //
			...
			for (var i:uint = 0; i < 10; i++) { // loop through rows of spinners
				...
				else if (randomNum > 0.33 && randomNum <= 0.5) { // decorative platforms
					this.builder.levelElementsArray.push([gy, -300, "PlatformNormal", 4]);
					this.builder.levelElementsArray.push([gy, -180, "PlatformNormal", 4]);
					this.builder.levelElementsArray.push([gy, -60, "PlatformNormal", 4]);
				}
				else if (randomNum > 0.5 && randomNum <= 0.67) {
					this.builder.levelElementsArray.push([gy, 300, "PlatformNormal", 4]);
					this.builder.levelElementsArray.push([gy, 180, "PlatformNormal", 4]);
					this.builder.levelElementsArray.push([gy, 60, "PlatformNormal", 4]);
				}
...
//			this.builder.levelElementsArray.push([this.builder.currentY * Constants.UnitHeight, Constants.PowerSettingDuplication, false]);
			...
			for (var i:uint = 0; i < 10; i++) { // loop through rows of spinners
				...
				this.builder.levelElementsArray.push([gy, gx, "Repulsor", 0, 0, 0, 120, (Math.random() * 2 + 1) * Math.PI / 180, true]);
...
//				}
		...
		private function generateStraightLine(difficulty:uint = 1):void { // straight line upward
			...
			while(this.builder.currentY < targetY) {
				...
				if (difficulty == 1) { // two straight lines at the left and right edges
					var randomNum:Number = Math.random();
					if (randomNum < 0.5) {
						if (randomNum < 0.05) this.builder.levelElementsArray.push([gy, -100, "StarRed"]);
						else this.builder.levelElementsArray.push([gy, -100, "Star"]);
						if (randomNum >= 0.45) this.builder.levelElementsArray.push([gy, 100, "StarRed"]);
						else this.builder.levelElementsArray.push([gy, 100, "Star"]);
						this.builder.levelElementsArray.push([gy, 0, "Coin"]);
					} else {
						this.builder.levelElementsArray.push([gy, -100, "Coin"]);
						this.builder.levelElementsArray.push([gy, 100, "Coin"]);
						this.builder.levelElementsArray.push([gy, 0, "Star"]);
					}
				}
				else if (difficulty == 2) {
					// left column
			...
			gy = this.builder.currentY * Constants.UnitHeight;
			if (difficulty == 1) {
				this.builder.levelElementsArray.push([gy, -100, "StarBlue"]);
				this.builder.levelElementsArray.push([gy, 100, "StarBlue"]);
			}
			else if (difficulty == 2 || difficulty == 3) {
				this.builder.levelElementsArray.push([gy, gx - 50, "StarBlue"]);
		...
		private function generateStarCrossLarge(difficulty:uint = 1):void {
			var gx:Number;
			if (difficulty == 1) gx = Math.random() * 400 - 200;
			else gx = Math.random() * 600 - 300;
			var gy:Number = this.builder.currentY * Constants.UnitHeight;
			var j:uint;
			var k:uint;
		...
		private function addStarBlueDark(gy, gx, velocityX:Number):void {
			if (Math.random() > 0.7) this.builder.levelElementsArray.push([gy, gx, "Star", 0, velocityX]);
		 ...
		 * Pairs of bouncers
		 */
		private function generateBouncersPairs():void {
			var gx:Number;
			var gy:Number;
			var targetY:int = this.builder.currentY + 60;
			while(this.builder.currentY < targetY) {
				gx = Math.random() * 400 - 200;
				this.builder.levelElementsArray.push([this.builder.currentY * Constants.UnitHeight, gx, "Bouncer"]);
				if (gx < 0) {
					this.builder.levelElementsArray.push([(this.builder.currentY + 1) * Constants.UnitHeight, gx - 170, "Bouncer"]);
				} else {
					this.builder.levelElementsArray.push([(this.builder.currentY + 1) * Constants.UnitHeight, gx + 170, "Bouncer"]);
				}
				...
				if (Math.random() < 0.4) {
					gx = Math.random() * 600 - 300
					gy = (this.builder.currentY + 2) * Constants.UnitHeight;
					this.builder.levelElementsArray.push([gy, gx, "StarRed", 0, 0, 0.5]);
				}
		...
		/**
		 * Ascending stair with given platform
		 */
		private function generateStairWithName(platformName:String, size:int = 0, stairHeight:uint = 1):void {
			if (Math.random() < 0.5) { //left to right ascending
				if (platformName == "Bouncer") {
					this.builder.levelElementsArray.push([(this.builder.currentY + 0 * stairHeight + 1) * Constants.UnitHeight, -470, platformName, size]);
				}
				this.builder.levelElementsArray.push([(this.builder.currentY + 0 * stairHeight) * Constants.UnitHeight, -300, platformName, size]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 0 * stairHeight + 2) * Constants.UnitHeight, -300, "Coin"]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 1 * stairHeight) * Constants.UnitHeight, -180, platformName, size]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 1 * stairHeight + 2) * Constants.UnitHeight, -180, "Coin"]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 2 * stairHeight) * Constants.UnitHeight, -60, platformName, size]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 2 * stairHeight + 2) * Constants.UnitHeight, -60, "Coin"]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 3 * stairHeight) * Constants.UnitHeight, 60, platformName, size]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 3 * stairHeight + 2) * Constants.UnitHeight, 60, "Coin"]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 4 * stairHeight) * Constants.UnitHeight, 180, platformName, size]);
				this.builder.levelElementsArray.push([(this.builder.currentY + 4 * stairHeight + 2) * Constants.UnitHeight, 180, "Coin"]);
				if (platformName == "Bouncer") {
					this.builder.levelElementsArray.push([(this.builder.currentY + 4 * stairHeight + 1) * Constants.UnitHeight, 350, platformName, size]);
				} else {
					this.builder.levelElementsArray.push([(this.builder.currentY + 5 * stairHeight) * Constants.UnitHeight, 300, platformName, size]);
				}
				this.builder.levelElementsArray.push([(this.builder.currentY + 5 * stairHeight + 2) * Constants.UnitHeight, 300, "Coin"]);
			...
			this.builder.currentY += 6 * stairHeight + 2;
		}
		...
		private function addRedStarsAtRandom(probability:Number):void {
			if (Math.random() < probability) {
				var gx:Number = Math.random() * 600 - 300;
				var gy:Number = this.builder.currentY * Constants.UnitHeight;
				if (gx < 0) {
					this.builder.levelElementsArray.push([gy, gx, "CoinBubble"]);
					this.builder.levelElementsArray.push([gy, gx, "StarRed", 0, 0, 0, 50, null, true]);
					this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0, 0, 0, 50, null, true, Math.PI / 2]);
					this.builder.levelElementsArray.push([gy, gx, "StarRed", 0, 0, 0, 50, null, true, Math.PI]);
					this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0, 0, 0, 50, null, true, Math.PI * 3 / 2]);
				} else {
					this.builder.levelElementsArray.push([gy, gx, "CoinBubble"]);
					this.builder.levelElementsArray.push([gy, gx, "StarRed", 0, 0, 0, 50, null, false]);
					this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0, 0, 0, 50, null, false, Math.PI / 2]);
					this.builder.levelElementsArray.push([gy, gx, "StarRed", 0, 0, 0, 50, null, false, Math.PI]);
					this.builder.levelElementsArray.push([gy, gx, "StarBlue", 0, 0, 0, 50, null, false, Math.PI * 3 / 2]);
				}
			}
		}
	}
